...
export function HomeMonthCategoryDonut({ ranks, currency = 'CNY' }: Props) {
  const { slices, total } = useMemo(() => {
    ...
    const all = top.map((r) => ({
      name: r.category_name || '未分类',
      total: r.total,
      count: r.tx_count
    }))
    if (restTotal > 0) {
      all.push({ name: '其他', total: restTotal, count: restCount })
    }
  ...
  const conic = useMemo(() => {
    ...
    const stops: string[] = []
    slices.forEach((s, i) => {
      const color = s.name === '其他' ? OTHER_COLOR : PALETTE[i % PALETTE.length]
        ...
        {slices.length === 0 ? (
          ...
          <div className="flex items-center gap-5">
            ...
            <ul className="min-w-0 flex-1 space-y-1.5">
              {slices.map((s, i) => {
                const color =
                  s.name === '其他' ? OTHER_COLOR : PALETTE[i % PALETTE.length]
